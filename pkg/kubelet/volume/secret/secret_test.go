...
func TestCanSupport(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Can't find the pluginin by name")
	}
...
func TestPlugin(t *testing.T) {
	...
	for key, _ := range secret.Data {
		dataHostPath := path.Join(volumePath, key)
		if _, err := os.Stat(dataHostPath); err != nil {
			t.Fatalf("SetUp() failed, couldn't find secret data on disk: %v", dataHostPath)
		} else {
